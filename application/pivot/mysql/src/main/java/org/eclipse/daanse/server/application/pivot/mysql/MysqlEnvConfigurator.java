...
/**
 * Registers the MySQL DataSource from the {@code DAANSE_JDBC_*} environment
...
public class MysqlEnvConfigurator {

    private static final Logger logger = LoggerFactory.getLogger(MysqlEnvConfigurator.class);
    ...
    public void activate() throws IOException {
        ...
        if (!missing.isEmpty()) {
            logger.error("Not configuring the MySQL DataSource, required environment variables are not set: {}",
                    missing);
            return;
        }
        ...
        logger.info("MySQL DataSource, connection pool and context configured from environment");
    }
