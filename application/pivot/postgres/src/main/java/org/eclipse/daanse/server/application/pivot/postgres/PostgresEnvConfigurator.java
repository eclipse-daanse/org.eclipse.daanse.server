...
/**
 * Registers the PostgreSQL DataSource from the {@code DAANSE_JDBC_*}
...
public class PostgresEnvConfigurator {

    private static final Logger logger = LoggerFactory.getLogger(PostgresEnvConfigurator.class);
    ...
    private static final String ENV_DBNAME = "DAANSE_JDBC_DBNAME";
    ...
    public void activate() throws IOException {
        List<String> missing = Env.missing(ENV_USER, ENV_PASSWORD, ENV_DBNAME);
        if (!missing.isEmpty()) {
            logger.error("Not configuring the PostgreSQL DataSource, required environment variables are not set: {}",
                    missing);
            return;
        }
        ...
        logger.info("PostgreSQL DataSource, connection pool and context configured from environment");
    }
