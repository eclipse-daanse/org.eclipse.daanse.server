...
/**
 * Registers the Oracle DataSource from the {@code DAANSE_JDBC_*} environment
 * variables and creates the BasicContext against it. Every attribute of
 * {@link DsConfig} is supported.
 */
@Component(immediate = true)
@RequireConfigurationAdmin
@RequireServiceComponentRuntime
public class OracleEnvConfigurator {

    private static final Logger logger = LoggerFactory.getLogger(OracleEnvConfigurator.class);

    private static final String DIALECT_NAME = "ORACLE";

    private static final String ENV_USER = "DAANSE_JDBC_USER";
    private static final String ENV_PASSWORD = "DAANSE_JDBC_PASSWORD";
    private static final String ENV_SERVICE_NAME = "DAANSE_JDBC_SERVICE_NAME";
    ...
    public void activate() throws IOException {
        List<String> missing = Env.missing(ENV_USER, ENV_PASSWORD);
        if (!missing.isEmpty()) {
            logger.error("Not configuring the Oracle DataSource, required environment variables are not set: {}",
                    missing);
            return;
        }
        if (Env.get(ENV_SERVICE_NAME).isEmpty() && Env.get(ENV_DATABASE_NAME).isEmpty()) {
            logger.error("Not configuring the Oracle DataSource, neither {} (service name) nor {} (SID) is set",
                    ENV_SERVICE_NAME, ENV_DATABASE_NAME);
            return;
        }
        ...
        logger.info("Oracle DataSource, connection pool and context configured from environment");
    }
